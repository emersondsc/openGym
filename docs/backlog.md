...
## BACKLOG-02 — Micro: Gerar microciclo da semana via app (app como interface, Hermes como motor) — CANCELADA/SUPERADA em 14/09/2026; o que sobreviveu é a publicação (spec v6)

- **Decisão do usuário (14/09/2026): este desenho está CANCELADO.** O que ele pedia — botão no app
  pedindo o micro ao assistente, stepper da Fase 2 dentro do app, `hermes_api.py` na 8091 e o proxy
  `/api/hermes/*` no nginx — foi decidido **fora**: não haverá "app como interface". O que sobreviveu é
  a **publicação da semana pelo assistente**, que virou a spec `docs/specs/spec_publicacao_micro.md`
  **v6** (os mesmos treinos atualizados, numa escrita só, dia só com autorização do usuário). É por
  essa spec que a implementação segue; tudo o que está abaixo fica como **histórico do desenho
  cancelado**.

- **Bloqueadores (atualizado 11/09/2026):** **nenhum.** O último bloqueador era a **BACKLOG-07** (API de rotinas), resolvida e no ar em 11/09/2026 — ver **Concluídos**. A **BACKLOG-05/06** caiu em 10/09, e com ela a **proibição de alvos divergentes entre rotinas** que a spec v5 impunha: o microciclo já pode prescrever cargas diferentes para o mesmo exercício em rotinas diferentes (está em uso no meso atual). Restam como **satélites de risco**, que não impedem o caminho feliz: **BACKLOG-08** (backup do Hermes — **concluída em 12/09/2026**, com o que sobrou desmembrado nos itens 10 a 14) e **BACKLOG-09** (concorrência real no sync — **implementado e medido em 12/09/2026**: o token, a releitura ao voltar e a revisão confirmada estão no ar; falta só a janela de 7 dias sem escrita sem token).
...
- **Investigar/propor (feito em 12/09/2026):** ~~token de concorrência real~~ — **implementado** como `S.rev` + `If-Match` por igualdade; ~~decidir o destino do `If-Match-State`~~ — **removido do desenho**; ~~decidir se o app deve re-buscar a base ao ganhar foco~~ — **decidido e implementado**: relê ao voltar para a frente (só `visibilitychange`, no máximo uma vez a cada 20 s, nunca empurrando, e sem rodar se houver push agendado), decidindo a adoção **por `rev`** e não por `_ts`; ~~medir o cenário do aceite~~ — **medido** (abaixo).

- **Aceite (da investigação) — cumprido, resta a janela:** ~~fluxo de sync atual descrito com os pontos exatos de carimbo de `_ts`~~ (feito) · ~~proposta de token com o custo no cliente e no servidor~~ (feito e implementado) · ~~decisão registrada sobre o `If-Match-State`~~ (feito: removido) · ~~cenário reproduzido (PWA aberto + escrita externa + edição local) com o resultado medido~~ (feito em 12/09/2026 — ver a medição abaixo). **Falta:** os **7 dias sem nenhuma escrita sem token**.
- **Medição do aceite (12/09/2026, em produção, com backup datado e sem alterar dado nenhum):**
  ```
  [og-state] op=put          actor=pwa-legacy ifMatch=77 rev=77->78    <- edição 1: o cliente publicado manda o token
  [og-state] op=put          actor=pwa-legacy ifMatch=78 rev=78->79    <- edição 2: mandou 78, que só podia vir da resposta anterior
  [og-state] op=put          actor=hermes     ifMatch=79 rev=79->80    <- escrita externa com o app aberto
  [og-state] op=put-rejected reason=stale     ifMatch=79 serverRev=80  <- a edição local foi RECUSADA, em vez de sobrescrever
  [og-state] op=put          actor=pwa-legacy ifMatch=80 rev=80->81    <- releu, mesclou e reenviou sozinho
  ```
  Estado final contra o backup: diferença **apenas** em `rev` (77→81) e `_ts` — 301 treinos e 6 rotinas idênticos. Nenhuma escrita sem token durante a medição.
- **O critério de saída anterior estava errado, e a medição mostrou.** O rótulo `actor: "pwa-legacy"` **não** distingue cliente antigo de novo: o servidor o aplica a qualquer requisição sem a assinatura do assistente, e o navegador nunca manda essa assinatura — medido, **72 de 72** linhas do registro, inclusive as duas de 16:31 já com o bundle novo no ar. Por decisão do usuário (12/09/2026) o `api/server.js` ganhou **9 linhas de instrumentação** (o campo `ifMatch` no registro + dois logs de escrita recusada); o critério passou a ser **7 dias sem nenhuma escrita com `ifMatch: "none"`**, e o contador começou limpo (0).
- **O que NÃO foi exercitado ao vivo:** a releitura ao voltar (RF-1) — ela depende de trocar de aba e voltar, e as ferramentas do agente não ativam uma aba já aberta (navegar de novo faria recarga, que é o pull de boot). Fica coberta pelos invariantes de teste; o passo 10 da spec é o que a exercita com o DevTools.
- **Satélite criado por esta medição (sem número ainda):** um **ajuste de configuração que perde a corrida é descartado em silêncio**. A mescla do `409` monta o estado a partir do servidor, então o ajuste local volta atrás sem aviso nenhum — medido ao vivo: o `keepAwake` terminou no valor do servidor. É **pré-existente** (a mescla de escalares é a de antes; o que esta rodada mexeu foi a de rotinas) e não é o que a U1 protege (plano, rotina e treino seguem preservados). Candidato a item próprio.

- **Refs:** `api/server.js` (`PUT /api/data`: guard por `If-Match`, campo `ifMatch` no registro, logs `op=put-rejected`) · `frontend/src/store/useStore.js` (`adoptRev`, `noticeIfChanged`, `pullOnReturn`, `pullState`) · `frontend/src/lib/plan-merge.js` (`planChanged`) · `docs/specs/spec_concorrencia_sync.md` (v4) · `docs/specs/spec_api_rotinas.md` · BACKLOG-02 e BACKLOG-07
...
- **Status (histórico):** aberto, sem urgência imediata e sem nenhuma vigilância.
- **O problema:** `/mnt/drivebackup` (215 GB) vivia perto do teto (91% usado) e **nenhum script media espaço**: o `island_healthcheck.sh` só testava se conseguia gravar um arquivo de prova (`READONLY_OR_FULL`), ou seja, avisava **depois** de o disco encher. O que ocupa: o arquivo do Drive antigo (185 GB, preservado de propósito na aposentadoria do espelho em 12/09), `island-vol` (6,3 GB), `apps` (1,6 GB) e `HermesLocal` (1,3 GB).
- **Resolução:** o `island_healthcheck.sh` passou a medir o espaço livre do continente **antes** de encher e a avisar por transição, igual ao que já fazia com o app:
  - Piso de **10 GB livres**, sobrescrevível por `LIVRE_MIN_GB` (mesmo desenho do `SNAPSHOT_MAX_H`). Medição com `df -Pk` (formato POSIX, unidade travada em blocos de 1K, imune ao `DF_BLOCK_SIZE` do ambiente).
  - **Mede só com o HD montado.** `df` de caminho desmontado cai no filesystem que o contém e mentiria "204 GB livres" — o guarda é `mountpoint -q`. Sem medição, o veredito é `desconhecido` e o **último estado é preservado** (o sensor não esquece que estava bom nem inventa alarme).
  - Veredito na **linha 5** do arquivo de estado (`ok`/`baixo`), sem renumerar as quatro linhas existentes; o arquivo antigo de 4 linhas (linha 5 vazia) conta como "nunca baixou do piso" e alerta na primeira queda.
  - Alerta no Telegram na **transição** para baixo (com o valor medido e o piso) e na recuperação, o que re-arma o alarme; enquanto segue abaixo do piso, silêncio. A linha do ciclo no log ganhou `livre=<n>GB`.
- **Provado ao vivo em 12/09/2026 (3 rodadas, com o piso forçado a 999 GB):** alerta de problema entregue; segunda rodada com o piso estourado **sem** mensagem nenhuma (transição, não estado); volta ao piso real gerando a mensagem de normalização e o estado de volta em `ok`. Nenhuma regressão nos outros modos (`--containers` e `--incidents` continuam saindo antes, e a linha do ciclo segue com `incidentes=`), e nenhum outro script lê o arquivo de estado.
- **A limpeza do cache de build foi feita e o resultado foi outro:** `docker builder prune` devolveu **7,386 GB**, mas para o **disco raiz** (195 → 201 GB), não para o HD. O cache vive em `/var/lib/containerd` (disco raiz) — `/mnt/drivebackup/docker/buildkit` está vazio, e o data-root no HD é só o ponto de montagem dos containers. **O HD ficou nos mesmos 19 GB livres**, então a margem do piso não mudou. As 9 imagens e os 4 containers em uso ficaram intactos (`opengym-api-1` e `opengym-web-1` seguem `healthy`).
- **Consequência honesta:** o espaço do HD é estrutural, não operacional. O único alavanca real é o arquivo do Drive antigo (185 GB) sair para outro disco, e isso está **decidido que não se toca**. As 4 imagens inativas (6,4 GB) também são do disco raiz: podá-las não ajuda este item e custa a capacidade de rollback. O sensor é a mitigação certa: avisa com 10 GB de folga, o que dá tempo de agir.
- **Aceite:** cumprido — o sensor avisa **antes** de o disco encher (era esse o critério), com histerese por transição e sem flood. Hoje: **19 GB livres, piso 10 GB**.
- **Refs:** `~/.hermes/scripts/island_healthcheck.sh` (`continent_livre_kb`, bloco "Transicao: espaco livre do continente") · `/mnt/drivebackup/{GoogleDrive,island-vol,apps,HermesLocal}` · `docker system df` · `RESTORE.md` (seção de saúde do continente)

### BACKLOG-11 — Incidente de falha de job morre no banco (ninguém é avisado) — **RESOLVIDO em 12/09/2026**

- **Status (histórico):** era a causa raiz que sobrou da lacuna 2 da BACKLOG-08.
- **O problema:** o agente detectava a falha de job corretamente e gravava em `cron_incidents`, mas **nada consumia** (14 incidentes, 0 reconhecidos, 0 fechados). Quatro dos oito jobs ativos entregavam em `local` e não tinham `failure_deliver`, então a falha deles não tinha para onde ir. Foi assim que o `snapshot.sh` falhou **10 dias seguidos** (31/08 a 09/09, `exit 128`) com cada falha gravada num arquivo de `cron/output/` que ninguém leu.
- **Decidido com o usuário (12/09/2026):** ser lembrado **enquanto** continuar falhando (um aviso por dia, não um só); o incidente fica pendente até ele dar `ack`, com o comando dentro da mensagem; e os **quatro** jobs mudos passam a avisar.
- **Resolução — duas frentes:**
  - **Aviso nativo:** `failure_deliver: telegram` nos quatro jobs que entregavam em `local` (`c75628767d63` Hermes Snapshot, `fe7ae09e86b4` sync-treino-diario-23h, `c7881494de97` hermes-local-backup, `2e74802a9fe1` update-date-daily), por `hermes cron edit`. A falha deles agora chega no celular na hora, pelo caminho do próprio agente.
  - **Rede independente:** o `island_healthcheck.sh` (cron do sistema, a cada 15 min) passou a ler o banco de incidentes **somente leitura** e a anunciar o que ninguém anunciou: (a) incidente preso em `detected` há mais de 60 min e dentro da janela de 48 h; (b) job ativo cuja **última** execução não conseguiu entregar o aviso (`last_delivery_error`). Mensagem com nome do job, motivo e o comando de reconhecer. Novo modo `--incidents` para espiar o que está alertável sem alertar e sem mexer em estado. A linha do ciclo no log ganhou `incidentes=<n>detected/<n>alerted`.
  - Detalhe que **quase** virou o mesmo silêncio de novo: o alvo do aviso **não** pode ser `origin` — o `Hermes Snapshot` tem `origin: null` (o desfecho vira `not_configured`: o incidente é marcado `alerted` e **nenhuma** mensagem sai) e o `hermes-local-backup` nasceu na WebUI. Daí `telegram` explícito, o mesmo alvo que o `alerta.sh` já usa. A spec traz uma verificação que roda antes e falha se algum dos quatro não estiver em `telegram`.
- **Provas (10 critérios de aceite, todos executados em 12/09/2026):** falha real de um job descartável chegou no Telegram e o incidente nasceu `alerted` (`last_delivery_error: None`); incidente vivo plantado numa cópia do banco gerou **exatamente um** alerta, sem repetir na segunda passada, com a chave `i:<id>` gravada só depois da entrega; incidente de 20 min foi ignorado pela carência; histórico de 10 dias não alertou; banco ausente, `jobs.json` ausente e job pausado degradaram em silêncio; `--incidents` não escreveu no log do ciclo nem em `.incidents-told`; `hermes cron doctor` sem problemas e o código do agente intacto (`HEAD` `e9bccc90a5e3`).
- **Rollback:** `hermes cron edit <id> --failure-deliver ''` (a CLI limpa o override; as falhas voltam a seguir o `deliver`, que é `local` = silêncio) e o backup do sensor em `~/.hermes/scripts/island_healthcheck.sh.bak.20260912-1423`.
- **O que ficou de fora, de propósito:** nenhuma edição no código do agente (o `hermes update` sobrescreve a árvore e o módulo de incidentes já faz o que precisa); nada de `ack` automático; nenhuma mensagem de recuperação (quem fecha é o humano); o histórico de agosto não é apagado nem reconhecido em massa.
- **Limitações conhecidas:** o vigia usa o **mesmo canal** do aviso nativo (a independência é em relação ao agendador, não ao Telegram); o silêncio sobre agosto depende da janela de 48 h, com cerca de um dia de margem; e o alerta da perna (b) repete no máximo uma vez por dia enquanto o campo continuar preenchido.
- **Refs:** `docs/specs/spec_incidentes_de_job.md` (spec v3, 464 linhas) · `~/.hermes/scripts/{island_healthcheck.sh,alerta.sh}` · `~/.hermes/cron/{executions.db,jobs.json}` · `~/.hermes/drive_backup/.incidents-told` · `RESTORE.md` (seção de incidentes)
...
*Atualizado 12/09/2026 (16): **BACKLOG-11 RESOLVIDA e movida para Concluídos.** A falha de job deixou de morrer no banco: os quatro jobs que entregavam em `local` ganharam `failure_deliver: telegram` (aviso imediato, pelo caminho do próprio agente) e o `island_healthcheck.sh` — que roda no cron do sistema, a cada 15 min — passou a ler o banco de incidentes em modo somente-leitura e a anunciar o que ninguém anunciou (incidente preso em `detected` há mais de 60 min; job ativo cuja última execução não entregou o aviso), com nome do job, motivo e o comando de `ack` dentro da mensagem. Modo novo `--incidents` para espiar sem alertar, e a linha do ciclo no log ganhou `incidentes=<n>detected/<n>alerted`. **O achado que quase repetiu o silêncio:** o alvo do aviso não podia ser `origin` — o `Hermes Snapshot` tem `origin: null` (o desfecho vira `not_configured`, o incidente é marcado `alerted` e nenhuma mensagem sai) e o `hermes-local-backup` nasceu na WebUI; daí `telegram` explícito. Os 10 critérios de aceite foram executados no Pi (mensagem real de uma falha de job descartável, incidente plantado numa cópia do banco gerando exatamente um alerta e sem repetir, carência de 60 min e janela de 48 h provadas, degradações silenciosas, `cron doctor` limpo e código do agente intacto). Spec: `docs/specs/spec_incidentes_de_job.md` (v3, 464 linhas). Segue aberto da família: **BACKLOG-10** (o snapshot ainda é agendado pelo agente) e **BACKLOG-16** (sensor de espaço em disco).*
*Atualizado 12/09/2026 (17): **BACKLOG-16 RESOLVIDA e movida para Concluídos.** O `island_healthcheck.sh` passou a medir o espaço livre do continente e a avisar **antes** de encher (piso de 10 GB, `LIVRE_MIN_GB` sobreponível, veredito na linha 5 do estado, alerta na transição e na recuperação), em vez de só descobrir o disco cheio pelo `READONLY_OR_FULL` depois do estrago. Verificado ao vivo nas três rodadas (abaixo do piso, repetição silenciosa, recuperação) e sem regressão nos modos `--containers`/`--incidents` nem no contador `incidentes=` da linha do ciclo. **Correção de premissa do item:** o `docker builder prune` devolveu 7,386 GB, mas para o **disco raiz** (195 → 201 GB) — o cache de build vive em `/var/lib/containerd`, não no HD, que segue com 19 GB livres. Ou seja: a limpeza foi feita e é boa higiene, mas **não** aumentou a folga do HD; a folga do HD só muda se o arquivo do Drive antigo (185 GB, intocável por decisão) sair para outro disco. Fecham-se assim os dois itens da família do backup que sobraram da BACKLOG-08: a 11 e a 16. Seguem abertos, fora dessa família: **BACKLOG-10** (o snapshot ainda é agendado pelo próprio agente), **BACKLOG-12** (protocolo de mutação em dois lugares), **BACKLOG-13** (cobertura do L1), **BACKLOG-14** (ensaio de restauração num Pi limpo) e **BACKLOG-15** (segredo real no `config.yaml`).*
*Atualizado 12/09/2026 (18): **BACKLOG-09 implementada e medida — falta só a janela de 7 dias.** Spec nova: `docs/specs/spec_concorrencia_sync.md` (v4; duas rodadas de revisão por subagente, 35 achados, mais a medição). O que entrou: o app **relê o plano ao voltar para a frente** (só `visibilitychange`, no máximo uma vez a cada 20 s, nunca empurrando, sem rodar com push agendado, e decidindo a adoção **por `rev`** em vez de `_ts` — por relógio o caso "aberto e sujo" nunca seria coberto, que era exatamente a pergunta que o item deixou aberta); o **aviso** de plano atualizado passou a ser por revisão (a chave antiga era gravada e nunca removida em lugar nenhum: o aviso saía no máximo uma vez na vida do aparelho) e sem atribuir autoria, conforme decisão do usuário, com tradução em PT que não existia; e o **cliente passou a guardar a revisão que o servidor confirma** — sem isso, todo salvamento depois do primeiro mandava base velha, apanhava `409` e reenviava: 3 requisições por edição. 252 testes verdes no Pi (eram 219).

**A medição do aceite foi feita em produção**, com backup datado e sem alterar dado nenhum: três edições reais no app com o bundle novo mostraram `ifMatch=77 rev=77->78` e depois `ifMatch=78 rev=78->79` **sem nenhuma recusa** (a prova de que o cliente lembra a revisão que o servidor devolveu); e, com uma escrita externa assinada como o assistente no meio, `op=put-rejected reason=stale ifMatch=79 serverRev=80` seguido de `ifMatch=80 rev=80->81` — **a edição local foi recusada em vez de sobrescrever, e o cliente se recuperou sozinho** (releu, mesclou, reenviou). O estado final difere do backup apenas em `rev` (77→81) e `_ts`: 301 treinos e 6 rotinas idênticos.
